...
func newProductCache() *gotest.Cache {
	...
	c.ICache = NewProductCache(&database.CacheType{
		CType: "redis",
		Rdb:   c.RedisClient,
	})
	return c
...
func Test_productCache_SetCacheWithNotFound(t *testing.T) {
	...
	record := c.TestDataSlice[0].(*model.Product)
	err := c.ICache.(ProductCache).SetPlaceholder(c.Ctx, record.ID)
	if err != nil {
		t.Fatal(err)
	}
	b := c.ICache.(ProductCache).IsPlaceholderErr(err)
	t.Log(b)
}

func TestNewProductCache(t *testing.T) {
	c := NewProductCache(&database.CacheType{
		CType: "",
	})
	assert.Nil(t, c)
	c = NewProductCache(&database.CacheType{

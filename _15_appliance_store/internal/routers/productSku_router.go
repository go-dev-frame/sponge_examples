...
func init() {
	...
	allRouteFns = append(allRouteFns,
		func(r *gin.Engine, groupPathMiddlewares map[string][]gin.HandlerFunc, singlePathMiddlewares map[string][]gin.HandlerFunc) {
			productSkuRouter(r, groupPathMiddlewares, singlePathMiddlewares, handler.NewProductSkuHandler())
		})
}

func productSkuRouter(
	r *gin.Engine,
	groupPathMiddlewares map[string][]gin.HandlerFunc,
	singlePathMiddlewares map[string][]gin.HandlerFunc,
	iService storeV1.ProductSkuLogicer) {
	storeV1.RegisterProductSkuRouter(
		r,
		groupPathMiddlewares,
		singlePathMiddlewares,
		iService,
		storeV1.WithProductSkuLogger(logger.Get()),
		storeV1.WithProductSkuHTTPResponse(),
		storeV1.WithProductSkuErrorToHTTPCode(
...
func productSkuMiddlewares(c *middlewareConfig) {
	// set up group route middleware, group path is left prefix rules,
	// if the left prefix is hit, the middleware will take effect, e.g. group route is /api/v1, route /api/v1/productSku/:id  will take effect
	// c.setGroupPath("/api/v1/productSku", middleware.Auth())

	// set up single route middleware, just uncomment the code and fill in the middlewares, nothing else needs to be changed
	//c.setSinglePath("POST", "/api/v1/productSku", middleware.Auth())    // Create productSku
	//c.setSinglePath("DELETE", "/api/v1/productSku/:id", middleware.Auth())    // DeleteByID delete productSku by id
	//c.setSinglePath("PUT", "/api/v1/productSku/:id", middleware.Auth())    // UpdateByID update productSku by id
	//c.setSinglePath("GET", "/api/v1/productSku/:id", middleware.Auth())    // GetByID get productSku by id
	//c.setSinglePath("POST", "/api/v1/productSku/list", middleware.Auth())    // List of productSku by query parameters
}

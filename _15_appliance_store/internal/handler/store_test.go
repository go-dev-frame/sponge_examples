...
func newStoreHandler() *gotest.Handler {
	...
	d.IDao = dao.NewStoreDao(d.DB, c.ICache.(cache.StoreCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &storeHandler{storeDao: d.IDao.(dao.StoreDao)}
	iHandler := h.IHandler.(storeV1.StoreLogicer)
	...
	testFns := []gotest.RouterInfo{
		{
			...
			HandlerFunc: func(c *gin.Context) {
				...
				if err != nil {
					response.Error(c, ecode.ErrCreateStore)
					return
				}
				response.Success(c)
			},
		},
		{
			...
			Path:     "/store/:id",
			HandlerFunc: func(c *gin.Context) {
				req := &storeV1.DeleteStoreByIDRequest{
					Id: utils.StrToUint64(c.Param("id")),
				}
				_, err := iHandler.DeleteByID(c, req)
				if err != nil {
					response.Error(c, ecode.ErrDeleteByIDStore)
					return
				}
				response.Success(c)
			},
		},
		{
			...
			Path:     "/store/:id",
			HandlerFunc: func(c *gin.Context) {
				req := &storeV1.UpdateStoreByIDRequest{}
				_ = c.ShouldBindJSON(req)
				req.Id = utils.StrToUint64(c.Param("id"))
				_, err := iHandler.UpdateByID(c, req)
				if err != nil {
					response.Error(c, ecode.ErrUpdateByIDStore)
					return
				}
				response.Success(c)
			},
		},
		{
			...
			Path:     "/store/:id",
			HandlerFunc: func(c *gin.Context) {
				req := &storeV1.GetStoreByIDRequest{
					Id: utils.StrToUint64(c.Param("id")),
				}
				_, err := iHandler.GetByID(c, req)
				if err != nil {
					response.Error(c, ecode.ErrGetByIDStore)
					return
				}
				response.Success(c)
			},
		},
		{
			...
			Path:     "/store/list",
			HandlerFunc: func(c *gin.Context) {
				req := &storeV1.ListStoreRequest{}
				_ = c.ShouldBindJSON(req)
				_, err := iHandler.List(c, req)
				if err != nil {
					response.Error(c, ecode.ErrListStore)
					return
				}
				response.Success(c)
			},
...
func Test_storeHandler_Create(t *testing.T) {
	h := newStoreHandler()
	defer h.Close()
	testData := &storeV1.CreateStoreRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Store))
...
func Test_storeHandler_DeleteByID(t *testing.T) {
	h := newStoreHandler()
	defer h.Close()
	testData := h.TestData.(*model.Store)
...
func Test_storeHandler_UpdateByID(t *testing.T) {
	h := newStoreHandler()
	defer h.Close()
	testData := &storeV1.UpdateStoreByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Store))
	testData.Id = h.TestData.(*model.Store).ID
...
func Test_storeHandler_List(t *testing.T) {
	h := newStoreHandler()
	defer h.Close()
	testData := h.TestData.(*model.Store)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &storeV1.ListStoreRequest{
		Params: &types.Params{
			Page:  0,
			Limit: 10,
	...
	// get error test
	err = httpcli.Post(result, h.GetRequestURL("List"), &storeV1.ListStoreRequest{Params: &types.Params{
		Page:  0,
		Limit: 10,
	}})
	assert.NoError(t, err)

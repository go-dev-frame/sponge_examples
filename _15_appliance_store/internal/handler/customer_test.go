...
func newCustomerHandler() *gotest.Handler {
	...
	d.IDao = dao.NewCustomerDao(d.DB, c.ICache.(cache.CustomerCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &customerHandler{customerDao: d.IDao.(dao.CustomerDao)}
	iHandler := h.IHandler.(storeV1.CustomerLogicer)
	...
	testFns := []gotest.RouterInfo{
		{
			...
			HandlerFunc: func(c *gin.Context) {
				req := &storeV1.CreateCustomerRequest{}
				_ = c.ShouldBindJSON(req)
				_, err := iHandler.Create(c, req)
				if err != nil {
					response.Error(c, ecode.ErrCreateCustomer)
					return
				}
				response.Success(c)
			},
		},
		{
			...
			Path:     "/customer/:id",
			HandlerFunc: func(c *gin.Context) {
				req := &storeV1.DeleteCustomerByIDRequest{
					Id: utils.StrToUint64(c.Param("id")),
				}
				_, err := iHandler.DeleteByID(c, req)
				if err != nil {
					response.Error(c, ecode.ErrDeleteByIDCustomer)
					return
				}
				response.Success(c)
			},
		},
		{
			...
			Path:     "/customer/:id",
			HandlerFunc: func(c *gin.Context) {
				req := &storeV1.UpdateCustomerByIDRequest{}
				_ = c.ShouldBindJSON(req)
				req.Id = utils.StrToUint64(c.Param("id"))
				_, err := iHandler.UpdateByID(c, req)
				if err != nil {
					response.Error(c, ecode.ErrUpdateByIDCustomer)
					return
				}
				response.Success(c)
			},
		},
		{
			...
			Path:     "/customer/:id",
			HandlerFunc: func(c *gin.Context) {
				req := &storeV1.GetCustomerByIDRequest{
					Id: utils.StrToUint64(c.Param("id")),
				}
				_, err := iHandler.GetByID(c, req)
				if err != nil {
					response.Error(c, ecode.ErrGetByIDCustomer)
					return
				}
				response.Success(c)
			},
		},
		{
			...
			Path:     "/customer/list",
			HandlerFunc: func(c *gin.Context) {
				req := &storeV1.ListCustomerRequest{}
				_ = c.ShouldBindJSON(req)
				_, err := iHandler.List(c, req)
				if err != nil {
					response.Error(c, ecode.ErrListCustomer)
					return
				}
				response.Success(c)
			},
...
func Test_customerHandler_Create(t *testing.T) {
	h := newCustomerHandler()
	defer h.Close()
	testData := &storeV1.CreateCustomerRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Customer))
...
func Test_customerHandler_DeleteByID(t *testing.T) {
	h := newCustomerHandler()
	defer h.Close()
	testData := h.TestData.(*model.Customer)
...
func Test_customerHandler_UpdateByID(t *testing.T) {
	h := newCustomerHandler()
	defer h.Close()
	testData := &storeV1.UpdateCustomerByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Customer))
	testData.Id = h.TestData.(*model.Customer).ID
...
func Test_customerHandler_List(t *testing.T) {
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &storeV1.ListCustomerRequest{
		Params: &types.Params{
			Page:  0,
			Limit: 10,
	...
	// get error test
	err = httpcli.Post(result, h.GetRequestURL("List"), &storeV1.ListCustomerRequest{Params: &types.Params{
		Page:  0,
		Limit: 10,
	}})
	assert.NoError(t, err)

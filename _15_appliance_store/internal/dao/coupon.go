...
type CouponDao interface {
	...
	CreateByTx(ctx context.Context, tx *gorm.DB, table *model.Coupon) (string, error)
...
func (d *couponDao) deleteCache(ctx context.Context, id string) error {
	if d.cache != nil {
...
func (d *couponDao) DeleteByID(ctx context.Context, id string) error {
...
func (d *couponDao) updateDataByID(ctx context.Context, db *gorm.DB, table *model.Coupon) error {
	...
	if table.Value.IsZero() == false {
		update["value"] = table.Value
	}
	if table.AmountCondition.IsZero() == false {
		update["amount_condition"] = table.AmountCondition
	}
...
func (d *couponDao) GetByID(ctx context.Context, id string) (*model.Coupon, error) {
	// no cache
	if d.cache == nil {
		record := &model.Coupon{}
	...
	if errors.Is(err, database.ErrCacheNotFound) {
		...
		val, err, _ := d.sfg.Do(id, func() (interface{}, error) {
			...
			// set cache
			if err = d.cache.Set(ctx, id, table, cache.CouponExpireTime); err != nil {
...
func (d *couponDao) GetByColumns(ctx context.Context, params *query.Params) ([]*model.Coupon, int64, error) {
	...
	if params.Sort != "ignore count" { // determine if count is required
		err = d.db.WithContext(ctx).Model(&model.Coupon{}).Where(queryStr, args...).Count(&total).Error
		if err != nil {
			return nil, 0, err
		}
		if total == 0 {
...
func (d *couponDao) CreateByTx(ctx context.Context, tx *gorm.DB, table *model.Coupon) (string, error) {
	err := tx.WithContext(ctx).Create(table).Error
	return table.ID, err
}

// DeleteByTx delete a record by id in the database using the provided transaction
func (d *couponDao) DeleteByTx(ctx context.Context, tx *gorm.DB, id string) error {
